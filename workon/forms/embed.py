...
class EmbedInput(forms.widgets.ClearableFileInput):
    template_name = 'workon/fields/_embed_input.html'
    change_message = "Changer"
    empty_message = "Changer"
    class Media:
        css = {
            'all': (
                'workon/forms/media.css',
            )
        }
        js = (
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(EmbedInput, self).__init__(*args, **kwargs)

    def render(self, name, value, attrs=None):
        # print "EmbedInput.render : ", name, type(value), value, self.is_initial(value)
        if value is None:
            value = ''

        field = super(EmbedInput, self).render(name, value, attrs=attrs)
    ...
    def value_from_datadict(self, data, files, name):
        # print 'EmbedInput.value_from_datadict : ', name, (files.get(name, None), data.get(name, None))
...
class EmbedField(forms.FileField):
    # widget = EmbedInput
    ...
    def __init__(self, *args, **kwargs):
        ...
        # widget = EmbedInput( attrs={
        #     'class': 'workon-media_field',
        #     # 'data-workon-media_field': self.image_field,
        #     # 'style': 'display:none;',
        # })
    ...
    def has_changed(self, initial, data):
        ...
        if data is None:
            return super(EmbedField, self).has_changed(initial, data)
        return True
    ...
    def clean(self, data, initial=None):
        ...
        if not data and initial:
            return initial
        return super(EmbedField, self).clean(data)