...
class HtmlInput(forms.Textarea):
    ...
    class Media:
        js = [
            workon.settings.WORKON_TINYMCE_URL,
            'workon/forms/html.js',
        ]
        css = {
            'all': ('workon/forms/html.css', )
        }
    ...
    def get_tinymce_config(self, name, attrs):
        config = copy.deepcopy(workon.settings.WORKON_TINYMCE_DEFAULT_CONFIG)
    ...
    def render(self, name, value, attrs=None):
        ...
        flatattrs['data-workon-html_input'] = self.get_config_json(config)


        if self.inline:

            html = [u"""<div class="workon-tinymce_inline">
