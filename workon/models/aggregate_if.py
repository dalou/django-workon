...
class AggregateIf(DjangoAggregate):
    def __init__(self, lookup, only=None, **extra):
        super(AggregateIf, self).__init__(lookup, **extra)
        self.only = only
        self.condition = None
...
class SumIf(AggregateIf):
    name = 'Sum'
    sql_klass = SqlSum


class CountIf(AggregateIf):
    name = 'Count'
    sql_klass = SqlCount


class AvgIf(AggregateIf):
    name = 'Avg'
    sql_klass = SqlAvg


class MaxIf(AggregateIf):
    name = 'Max'
    sql_klass = SqlMax


class MinIf(AggregateIf):
    name = 'Min'
    sql_klass = SqlMin