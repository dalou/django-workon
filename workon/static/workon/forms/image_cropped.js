...
{
    if(!$container[0].workon_image_cropped)
    {
        $container[0].workon_image_cropped = true;
        self = this;
        self.$container = $container
        self.$input = self.$container.find('input[type=file]');
        self.$img = self.$container.find('.workon-image_cropped-image img');

        if(self.$input.length && self.$input.attr('id').indexOf('__prefix__') == -1)
        {
            self.$form = $($input[0].form);
            if(self.$form.attr('enctype') != 'multipart/form-data')
            {
                self.$form.attr('enctype', 'multipart/form-data')
            }
            self.$container.on('click', '.workon-image_cropped-change', function()
            {
                self.$input.click();
            });
            self.$container.on('click', '.workon-image_cropped-remove', function()
            {
                self.$container.find('.workon-image_cropped-image').removeClass('active');
                self.$container.find('.workon-image_cropped-empty').addClass('active');
                self.$container.find('input[type=checkbox]').prop('checked', true);
            });

            self.$input.on("change", function(e)
            {
                self.input_change(this.files);
            });

            // if($input.parent().find('a').length) {
            //     REPLACE($input, $input_cropped, $input.parent().find('a').attr('href'));
            // }
...
{
    self = this;
    self.$container.find('.workon-image_cropped-image img').attr('src', src);
    self.$container.find('.workon-image_cropped-image').addClass('active');
    self.$container.find('.workon-image_cropped-empty').removeClass('active');
    self.$container.find('input[type=checkbox]').prop('checked', false);

}

fieldsBundleImageCropped.prototype.input_change = function(self)
{
    self = this;
    var files = self.$input[0].files;
    //delete_input = $('#%(name)s-clear_id');
    ...
    else
    {
        ...
        {
            return function(e) {
                self.replace_image(e.target.result);
                //APPLY($input, $input_cropped);
            };
        })(file);
...
{
    $(document).on('mouseenter', '.workon-image_cropped', function()
    {
        new fieldsBundleImageCropped($(this));
    });


    CLASS_VAR = 'workon-croppedimage';
    DATA_VAR = '[data-'+CLASS_VAR+']';
    APPLY = function($input, $input_cropped, src, $img, $container)
    {
        var coords = $input_cropped.val();

        coords = coords.split('?');
        if(coords.length >= 2) { coords = coords[1]; }
        ...
        $container = $input_cropped[0].$workon_cropped_image_container;
        if($container)
        {
            ...
            }).on('change.cropper', function(data) {

                data = $img.cropper('getData', true);
                var val = Math.round(data.x)+ '_'+ Math.round(data.y) +'_'+ Math.round(data.width) + '_'+ Math.round(data.height)
                $input_cropped.val(val);

            }).on('built.cropper', function(data) {
                console.log(coords)
            });
        }
        $(window).resize(function() {
            //$container.find('.cropper-container').css({ width: '100%' }).css({ width: '' })
        })
    ...
    {
        // $img = $input_cropped.parent().find('img')
        var $container = $input_cropped[0].$workon_cropped_image_container;
        if($container)
        {
        ...
        $input_cropped[0].$workon_cropped_image_container = $container;
        ...
        // if(data.$preview) {
        //     data.$preview.each(function(i, preview) {
        //         preview.CROPPABLE_PRE_IMG.attr('src', src);
        //     });
        // }
    }

    CHANGE_IMAGE = function($input, $input_cropped, files, delete_input){


    }


    INIT = function($input_cropped)
    {
        if(!$input_cropped[0].is_croppable) {

            $input = $('#id_'+$input_cropped.data(CLASS_VAR));
            console.log($input_cropped, $input)

            if($input.length && $input.attr('id').indexOf('__prefix__') == -1)
            {
                $input.on("change", function(e){

                    CHANGE_IMAGE($input, $input_cropped, this.files);
                });
            ...
            $input_cropped[0].is_croppable = true;


        }
    }

    $(DATA_VAR).each(function() {
        INIT($(this));
