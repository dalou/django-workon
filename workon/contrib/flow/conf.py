...
"""
FLOW_ENABLED = bool(int(getattr(settings, 'WORKON_FLOW_ENABLED', "0")))

"""
A boolean that allow Flow app debug printing
"""
FLOW_DISCONNECTED_ENABLED = bool(int(getattr(settings, 'WORKON_FLOW_DISCONNECTED_ENABLED', "0")))

"""
A boolean that allow Flow app debug printing
"""
FLOW_DEBUG = bool(int(getattr(settings, 'WORKON_FLOW_DEBUG', "1")))
...
"""
FLOW_INITIAL_URL = getattr(settings, 'WORKON_FLOW_INITIAL_URL', "/flow/")

"""
A boolean that allow Flow app debug printing
"""
FLOW_WS_ENABLED = bool(int(getattr(settings, 'WORKON_FLOW_WS_ENABLED', "1")))

FLOW_WS_URL = getattr(settings, 'WORKON_FLOW_WS_URL', '/ws/')

FLOW_WS_CONNECTION = getattr(settings, 'WORKON_FLOW_WS_CONNECTION', {
    'host': 'localhost',
    'port': 6379,
    'db': 0,
    'password': None,
...
"""
FLOW_WS_PREFIX = getattr(settings, 'WORKON_FLOW_WS_PREFIX', "flow")

"""
The time in seconds, items shall be persisted by the Redis datastore.
"""
FLOW_WS_EXPIRE = int(getattr(settings, 'WORKON_FLOW_WS_EXPIRE', 3600))
...
"""
FLOW_WS_HEARTBEAT = getattr(settings, 'WORKON_FLOW_WS_HEARTBEAT', '--heartbeat--')
...
database!
"""
FLOW_WS_ALLOWED_CHANNELS = getattr(settings, 'WORKON_FLOW_WS_ALLOWED_CHANNELS', None)
...
"""
FLOW_WS_PROCESS_REQUEST = getattr(settings, 'WORKON_FLOW_WS_PROCESS_REQUEST', None)
