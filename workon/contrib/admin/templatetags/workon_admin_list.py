...
def paginator_number(cl, i):
    ...
    """
    if i == DOT:
        return mark_safe('<li class="disabled"><a href="#" onclick="return false;">..' \
               '.</a></li>')
    elif i == cl.page_num:
        return mark_safe(
...
@register.simple_tag
def workon_admin_list_filter_select(cl, spec):
...
def result_row_attrs(context, cl, row_index):
    ...
    workon_admin_row_attributes = getattr(cl.model_admin, 'workon_admin_row_attributes', None)
    if not workon_admin_row_attributes:
        return dict_to_attrs(attrs)

    instance = cl.result_list[row_index]

    # Backwards compatibility for workon_admin_row_attributes without request argument
    args = getargspec(workon_admin_row_attributes)
    if 'request' in args[0]:
        new_attrs = workon_admin_row_attributes(instance, context['request'])
    else:
        new_attrs = workon_admin_row_attributes(instance)
    ...
    if not isinstance(new_attrs, dict):
        raise TypeError('"workon_admin_row_attributes" must return dict. Got: %s: %s' %
...
def cells_handler(results, cl):
    ...
    """
    workon_admin_cell_attributes = getattr(cl.model_admin, 'workon_admin_cell_attributes', None)
    if not workon_admin_cell_attributes:
        return results
    ...
    for row, result in enumerate(results):
        ...
        for col, item in enumerate(result):
            ...
            attrs = copy(workon_admin_cell_attributes(instance, field_name))
            if not attrs:
                continue

            # Validate
            if not isinstance(attrs, dict):
                raise TypeError('"workon_admin_cell_attributes" must return dict. '
                                'Got: %s: %s' % (
